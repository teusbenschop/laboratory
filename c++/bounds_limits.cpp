...
void demo()
{
    ...
    {
        const auto iter = std::ranges::lower_bound(container, 3);
        const auto index = std::distance(container.begin(), iter);
        assert(index == 2);
    }
    {
        const auto iter = std::ranges::upper_bound(container, 3);
        const auto index = std::distance(container.begin(), iter);
        assert(index == 5);
    }
}
