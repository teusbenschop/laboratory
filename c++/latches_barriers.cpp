...
static void output(Args...args) {
    if (print) {
        ...
        (void(std::cout << std::forward<Args>(args) << " "), ...);
        std::cout << std::endl;
    }
}
...
{
    {
        // The latch starts with a given count.
        ...
        for (auto i = 0; i < n_threads; ++i)
        {
            ...
            {
                output("Thread", std::this_thread::get_id(), "decreases latch and waits at it");
                latch.arrive_and_wait(); // Or just count_down();
                std::this_thread::sleep_for(std::chrono::milliseconds(1));
                output("Thread", std::this_thread::get_id(), "gets past latch");
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
            });
        }

        output("Main thread waits at latch till threads have counted it down to zero");
        latch.wait();
        output("Main thread gets past the latch");
    }
...
void demo()
{
...

    {
        ...
        // It checks whether all dice currently have a six.
        auto done{false};
    ...
    output("Number of turns to have all dice on 6:", n_turns);
}
}